...
bool ArmorTrack::Lock_Armor(std::vector<Armor> &Armors, double dt) {
    ...
    if(!Armors.empty()){
        ...
        /** 同时出现两个装甲板 */
        if(Armor_number == 2){
            double Track_Armor_diff = abs(Track_Armor[0].world_position[1]-Track_Armor[1].world_position[1]);
            // 两装甲板Y轴距离小于阈值
            if(Track_Armor_diff < 0.05) Long_Short_OK = true;
            // 进入采集模式
        ...
        if(Data_capture_OK){
            ...
            double B_1 = Track_Armor[0].world_position[1]-K_1*Track_Armor[0].world_position[0];
            // 右装甲板一次函数
            double angle_2 = -(90.0-abs(Track_Armor[1].R[1])*angle)*(CV_PI/180.0f);
            double K_2 = tan(angle_2);
            double B_2 = Track_Armor[1].world_position[1]-K_2*Track_Armor[1].world_position[0];
...
//            cv::waitKey(0);
    ...
    State_solve(match);
                                            // TODO：解决高低跳变的两个方法  帧率和测量误差引起的判断错误
                                            // TODO: 第一个方法，需要调整跳变阈值new_old_threshold(这个不好调)
                                           // TODO: 第二个方法，误判时造成高低问题,可以通过高低差的1/2或1/3判断是否要改变状态(优先方法)
                                           // TODO： 最终解决方法,将 diff 改成x轴上装甲板的距离差
    /** 识别到两个装甲板且处于跟踪状态 */
...
//                        std::cout << "new_armors:" << new_armors << std::endl;
    ...
    for (int i = 0; i < 4; i++)
    {
        line(_src, detect_armor.armor_pt4[i], detect_armor.armor_pt4[(i + 1) % 4], CV_RGB(0, 255, 255), 2);
        circle(_src,detect_armor.center,4,CV_RGB(0, 255, 255),-1);
    }
    // 输出角度
    std::string yaw_angle = std::to_string(enemy_armor.R[1]*180.0f/CV_PI);
    cv::putText(_src,"yaw:"+yaw_angle,cv::Point(0,120),cv::FONT_HERSHEY_SIMPLEX, 1,cv::Scalar(255, 255, 0),2,3);
    // 输出陀螺状态
    std::string Spin_State;
    if(spin_status_map[tracking_id] == UNKNOWN) Spin_State = "UNKNOWN";
    else if(spin_status_map[tracking_id] == CLOCKWISE)Spin_State = "CLOCKWISE";
    else if(spin_status_map[tracking_id] == COUNTER_CLOCKWISE)Spin_State = "COUNTER_CLOCKWISE";
    cv::putText(_src,"Spin_State:"+Spin_State,cv::Point(0,40),cv::FONT_HERSHEY_SIMPLEX, 1,cv::Scalar(255, 255, 0),2,3);
    // 输出当前跟踪ID
    cv::putText(_src,"Tracker_ID:"+ std::to_string(tracking_id),cv::Point(0,80),cv::FONT_HERSHEY_SIMPLEX, 1,cv::Scalar(255, 255, 0),2,3);
    // 输出陀螺分数
    for (auto score = spin_score_map.begin(); score != spin_score_map.end();) {
        cv::putText(_src,"Spin_Score:"+ std::to_string((*score).second),cv::Point(0,160),cv::FONT_HERSHEY_SIMPLEX, 1,cv::Scalar(255, 255, 0),2,3);
        score++;
    }
...
//        Long_axes = (Data_info[0][0] + Data_info[1][0] + Data_info[2][0])/max_size;
//        Short_axes = (Data_info[0][1] + Data_info[1][1] + Data_info[2][1])/max_size;
//        std::cout <<  Long_axes << "          " << Short_axes << std::endl;
