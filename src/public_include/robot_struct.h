...
struct BuffTarget {
    ...
    cv::RotatedRect buff_rect;              // 能量机关的旋转矩形
    cv::Point2f buff_center;              //能量机关目标的中心点
    cv::Point2f r_center;                // R标的中心点
    cv::Rect boundingRect;               // 边缘矩形
    cv::Point2f AS_center_input[4];   //AS四点的输入 顺序确定
    cv::Point2f AS_pre_input[4];       // AS预测四点
    std::vector<cv::Point2f> Val_points;
    double distance;                 //距离
    float target_angle;              //大能量机关击打点角度
    float leaf_angle;               //叶子中心角度
