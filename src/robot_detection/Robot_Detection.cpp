...
if(Vision_data.mode == 0x21)
{
    ...
    // 开始拟合圆心
    if(AO_OK) {
        AO.Angle_Speed = Tracker.O_predicted[0]; 
...
#ifndef temp
    // 整车观测(跟踪情况和当前跟踪车辆的观测情况)
    bool AO_OK = (Tracker.tracker_state == TRACKING) && T`racker.OB_Track[Tracker.tracking_id].is_initialized;
    // 开始拟合圆心
    if(AO_OK) {
        ...
        if(Tracker.Spin_State() == COUNTER_CLOCKWISE){
            // std::cout << AO.spin_angle*(180.0f/CV_PI) << "   " << AO.Left_Armor_angle << std::endl;
