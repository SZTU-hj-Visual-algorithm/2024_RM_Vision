...
//        double diff = (OB_Track[tracking_id].last_armor.world_position - enemy_armor.world_position).norm();
        ...
        if(diff > new_old_threshold && matched_armor.id == tracking_id ){
            ...
            // 计算ms的角速度   TODO:钉死图像处理速度为24ms？可能需要调整
            Angle_Speed /= 24;  
            std::cout << "Angle_Speed:" << Angle_Speed << std::endl;
            // Angle_Speed = 0.21;  //固定转速(拍视频使用)

            // 对一些不正确的数据进行处理
            if(!isnan(Angle_Speed) && !isinf(Angle_Speed) && Angle_Speed != 0 && Angle_Speed < 0.65){
                // 卡尔曼滤波部分
                OR_KF.setF(test_dt);
                // 预测步
                O_predicted = OR_KF.predict();
                // 更新步
                OR_KF.update(Angle_Speed);
                std::cout << "O_predicted:" << O_predicted[0] << std::endl;
            }
            std::cout << "-----------------------" << std::endl;
            ...
            // 神经网络
            #ifdef Neural_Mode
                old_angle = AS.Armor_Angle(OB_Track[tracking_id].last_armor);
                new_angle = AS.Armor_Angle(enemy_armor);
            #endif //Neural_Mode
...
void ArmorTrack::show() {
    ...
    cv::putText(src,"Angle_Speed:"+ std::to_string(Angle_Speed),cv::Point(850,460),cv::FONT_HERSHEY_SIMPLEX, 1,cv::Scalar(255, 255, 0),2,3);
