#include "../include/Neural_Buff_Detection.h"
...
// 参数读取部分
NeuralBuffDetector::NeuralBuffDetector() {
    //============================= 参数文件路径 =============================
    std::string package_path = ros::package::getPath("robot_NBDetection");
    cv::FileStorage fs(package_path + "/SetParam_File/Neural_BUFF_data.yaml", cv::FileStorage::READ);
 ...
 * @param frame 输入的原始图像。
 */
std::vector<NBDetection> NeuralBuffDetector::detect(Mat & frame) {
...
//    infer_request.wait();
    ...
    float* NBDetections = output_tensor.data<float>();
    this->frame_w = frame.cols;
    this->frame_h = frame.rows;

    return this->postprocess_img(frame, NBDetections, output_shape);
}
 ...
 */
void NeuralBuffDetector::initial_model() {
 ...
 * @param frame 输入的原始图像。
 */
void NeuralBuffDetector::preprocess_img_letterBox(cv::Mat &frame) {
    try {
 ...
 * @param NBDetections 模型的原始输出，包含检测到的对象的信息。
 * @param output_shape 模型输出的维度信息。
 */

std::vector<NBDetection> NeuralBuffDetector::postprocess_img(cv::Mat &frame, float *NBDetections, ov::Shape &output_shape) {
    // 模型输出的维度，其中行数为属性数量，列数为检测结果数量
    //int num_attributes = output_shape[1]; // 属性数量，这个模型是50个, 为什么用不上注释了呢?因为下面所有属性的提取我们都是写好的,用不上这个值来循环
    int num_NBDetections = output_shape[2]; // 检测结果数量，这个模型是8400个检测结果
...
//
//    for (int i = 0; i < num_NBDetections; ++i) {
//        float sum_score = 0.0;
//
//        // 对每个检测结果的类别置信度求和
//        for (int j = 0; j < 36; ++j) { // 假设有36个类别
//            sum_score += NBDetections[i + (4 + j) * num_NBDetections];
...
//            int class_id = -1;
//            for (int j = 0; j < 36; ++j) { // 假设有36个类别
//                float score = NBDetections[i + (4 + j) * num_NBDetections];
//                if (score > max_score) {
...
//        }
        ...
        if (max_score > confThreshold) {
            float cx = NBDetections[i]; // 第0行到第8399行是cx
            float cy = NBDetections[i + num_NBDetections]; // 第8400行到第16799行是cy
            float ow = NBDetections[i + 2 * num_NBDetections]; // 宽 16800 ~ 25199
            float oh = NBDetections[i + 3 * num_NBDetections]; // 高
            // 计算边界框的左上角点和宽高
            ...
            for (int k = 0; k < 5; ++k) {
                float kpt_x = ((NBDetections[i + (detectPointIndex + k * 2) * num_NBDetections] - this->dx) * this->rx);
                float kpt_y = ((NBDetections[i + ((detectPointIndex + 1) + k * 2) * num_NBDetections] - this->dy) * this->ry);
...
std::vector<Armor> NeuralBuffDetector::neuralArmorGrade(const std::vector<NBDetection>& candidateArmors) {
 ...
 *
 * @param frame 输入的图像，将在其上绘制检测结果。
 * @param NBDetections 经过postprocess处理后的输出，是可用的装甲板的信息。
 */
void NeuralBuffDetector::drawNBDetections(cv::Mat &frame, const std::vector<NBDetection> &NBDetections) {

    for (const auto &NBDetection: NBDetections) {
        auto box = NBDetection.box;
        int class_id = NBDetection.class_id;
        ...
        std::string classString =
                coconame[NBDetection.class_id] + ' ' + std::to_string(NBDetection.confidence).substr(0, 4);
        ...
        for (const auto &kpt: NBDetection.keyPoint) {
...
void NeuralBuffDetector::drawArmors(cv::Mat &frame, const std::vector<Armor> &armors) {
