...
#ifndef NeuralBuffDetector_Neural_Buff_NBDetection_H
#define NeuralBuffDetector_Neural_Buff_NBDetection_H
...
// 神经网络
class NeuralBuffDetector {
public:
	NeuralBuffDetector();
	~NeuralBuffDetector();
    std::vector<NBDetection> detect(cv::Mat& frame);
...
private:
	...
	// 图像处理
    std::vector<NBDetection> postprocess_img(cv::Mat& frame, float * NBDetections, ov::Shape & output_shape);
	// 装甲板打分
    std::vector<Armor> neuralArmorGrade(const std::vector<NBDetection>& candidateArmors);


	// 模型的检测结果可视化
    void drawNBDetections(cv::Mat& frame, const std::vector<NBDetection>& NBDetections);
	// 绘制装甲板
...
#endif //NeuralBuffDetector_Neural_Buff_NBDetection_H
