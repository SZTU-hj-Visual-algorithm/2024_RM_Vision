...
void Init(){
    ...
    strcpy(Camera_name,((std::string)fs["Camera_R"]).data());
...
int main(int argc, char *argv[]){
    ...
    // 初始化节点
    ros::init(argc, argv, "USB_Camera_R");

    // 创建句柄
    ros::NodeHandle nh;

    ros::Publisher usb_camera_pub = nh.advertise<robot_msgs::usb_camera>("/USB_Camera/PTZ_R",1);
