...
void Init(){
    ...
    strcpy(Camera_name,((std::string)fs["Camera_F"]).data());
...
int main(int argc, char *argv[]){
    ...
    // 初始化节点
    ros::init(argc, argv, "USB_Camera_F");
    ...
    ros::Publisher usb_camera_pub = nh.advertise<robot_msgs::usb_camera>("/USB_Camera/PTZ_F",1);
    ...
    while (ros::ok())
    {
        ...
        // 测试目前没有大问题
        ROS_INFO("X:%f Y:%f Z:%f",Tracker.enemy_armor.world_position[0],Tracker.enemy_armor.world_position[1],Tracker.enemy_armor.world_position[2]);
        ROS_INFO("Yaw:%f Main_yaw:%f",Tracker.Solve_yaw,main_yaw);
        ros::spinOnce();
    }
